...
# https://raw.githubusercontent.com/luozhouyang/machine-learning-notes/master/transformer/transformer_pytorch.py
#
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, model_dim, max_seq_len):
        ...
        position_encoding = np.array([[
            pos / np.pow(10000, 2.0 * (j // 2) / model_dim)
...
class Encoder(nn.Module):
    def __init__(self,
        ...
        self.seq_embedding = nn.Embedding(
            vocab_size + 1, model_dim, padding_idx=0)
...
def sequence_mask(seq):
    ...
    mask = torch.triu(
        torch.ones((seq_len, seq_len), dtype=torch.uint8), diagonal=1)
...
def padding_mask(seq_k, seq_q):
    ...
    # `PAD` is 0
    pad_mask = seq_k.eq(0)
    pad_mask = pad_mask.unsqueeze(1).expand(-1, len_q,
                                            -1)  # shape [B, L_q, L_k]
    return pad_mask
